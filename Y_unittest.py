...
class TestYConversion(unittest.TestCase):
    def test_y_complex_conversion(self, i, Y):
        filename = f'psi_phi_orth_y/iteration_{i}.csv'
        ...
        cosine_similarity = np.dot(y_prime.flatten(), Y.flatten()) / (np.linalg.norm(y_prime.flatten(), ord=2) * np.linalg.norm(Y.flatten(), ord=2))
